...
start_address = process.env.HUBOT_YELP_SEARCH_ADDRESS or "10012"
radius = process.env.HUBOT_YELP_SEARCH_RADIUS or 2000
sort = process.env.HUBOT_YELP_SORT or 0
default_suggestion = process.env.HUBOT_YELP_DEFAULT_SUGGESTION or "Hans"
...
lunchMe = (msg, query, random = true) ->
  ...
  yelp.search category_filter: "restaurants", term: query, radius_filter: radius, sort: sort, limit: 20, location: location, (error, data) ->
    if error != null
      return msg.send "There was an error searching for #{query}. Just go to #{default_suggestion}!"

    if data.total == 0
      return msg.send "I couldn't find any #{query} for you. Just go to #{default_suggestion}!"
